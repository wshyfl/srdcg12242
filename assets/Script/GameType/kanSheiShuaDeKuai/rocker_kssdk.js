cc.Class({
    ...
    update (dt) {
        ...
        let dis = this.dir.mul(this.Player.Speed * ratio);
        this.Player.setPosition(this.Player.position.add(dis));
        if(this.Player.x<-266){
            this.Player.x = -266;
        }
        else if(this.Player.x>262){
            this.Player.x = 262;
        }
        if(this.Player.y<-317){
            this.Player.y = -317;
        }
        else if(this.Player.y>362){
            this.Player.y = 362;
        }
    },
    onDestroy() {
    ...
    onTouchStart(event) {
        ...
        this.Player.getComponent("bluePlayer_kssdk").onPlayerMove("走")
    ...
    onTouchEnd(event) {
        ...
        this.Player.getComponent("bluePlayer_kssdk").onPlayerMove("待机")
