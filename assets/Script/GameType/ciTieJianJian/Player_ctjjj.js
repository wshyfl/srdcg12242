...
cc.Class({
    ...
    update(dt) {
        ...
        if (this.coinNode) {
            ...
            this.coinNode.angle = this.node.angle;
        }
    ...
    onAI(){
        if(this.node.name == "redPlayer") return
        if(!AD.isAI) return
        if(!AD.ctjjj.GameStart) return
        if(AD.Game.GameOver) return
        
        var angle = 0;
        if(this.haveCoin){
            if(this.coinNode.type =="炸弹"){
                angle = Tools.getAngle(this.node.parent, cc.v2(0, -300))
            }
            else{
                angle = Tools.getAngle(this.node.parent, cc.v2(0, 288))
            }
        }
        else{
            angle = Tools.getAngle(this.node.parent, AD.ctjjj.coinPanel.children[0]);
        }
        this.node.angle = angle +180;
        this.node.parent.x += 2.5 * Math.sin(Tools.angleToRadian(-angle + 180));
        this.node.parent.y += 2.5 * Math.cos(Tools.angleToRadian(-angle + 180));
    },
    onTip(type,num){
        if(type == "red"){
